...
import TasteTasterModal from '@/components/taste-taster/TasteTasterModal';

export default function HomePage() {
  const [showTasteTaster, setShowTasteTaster] = useState(false);

  useEffect(() => {
    // Check if the taste profile has been generated in this session
    if (sessionStorage.getItem('tasteProfileGenerated') !== 'true') {
      setShowTasteTaster(true);
    }
  }, []);

  const handleCloseTasteTaster = () => {
    setShowTasteTaster(false);
  };

  return (
    <>
      {showTasteTaster && <TasteTasterModal onClose={handleCloseTasteTaster} />}
